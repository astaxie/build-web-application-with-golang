...
- pkg 编译后生成的文件（比如：.a）
- bin 编译后生成的可执行文件（为了方便，可以把此目录加入到 $PATH 变量中），如果设置了$GOBIN，那么编译好的可执行文件放在$GOBIN目录下。
...
如何安装该应用，进入该目录执行`go install`,那么在$GOPATH/bin/下（如果设置了$GOBIN，那么在$GOBIN下）增加了一个可执行文件mathapp,这样可以在命令行输入如下命令就可以执行

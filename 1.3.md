...
##go doc
 ...
 通过命令在命令行执行 godoc -http=:端口号 比如( godoc -http=:8080).然后在浏览器中打开 127.0.0.1:8080 你将会看到一个 golang.org的本地copy版本,通过它你可以查询pkg文档等其他内容，如果你设置了GOPATH,在pkg分类下,不但会列出标准包的文档，还会列出你本地GOPATH中所有项目的相关文档,这对于经常墙奸的用户来说,是一个不错的选择。   

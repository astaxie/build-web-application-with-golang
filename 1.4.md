...
##LiteIDE
  ...
  LiteIDE是专门为Go语言开发的集成开发环境（IDE），由visualfc编写。支持语法高亮、自动补全、项目管理、集成构建、GDB调试等功能。[下载地址](http://code.google.com/p/golangide/downloads/list)

  请根据自己的系统下载相应的发行版本。Windows和Ubuntu系统可直接打开bin下面的liteide；Mac则需通过LaunchPad打开liteide。

...
接下来我们举一个简单的例子，模拟一个客户端用基于HTTP协议的请求去连接一个Web服务器。要发送一个简单的http请求头，类似如下代码：
...
通过上面的代码我们可以看出：首先程序接受用户的输入值作为参数传入`net.ResolveTCPAddr`获取了一个tcpaddr,然后将tcpaddr传入DialTCP后开始创建一个TCP连接，通过这个TCP连接来发送请求信息，最后`ioutil.ReadAll`读取TCP连接里的全部字符串流，也就是服务器反馈信息。

###TCP server
上面我们编写了一个TCP的客户端程序，也可以通过net包来创建一个服务器端程序。在服务器端我们需要绑定服务器到指定的非激活端口，并监听这个端口。当监听端口有客户端请求到达的时候就可以接收客户端的连接请求。net包中有相应的函数，函数定义如下：

	func ListenTCP(net string, laddr *TCPAddr) (l *TCPListener, err os.Error)
	func (l *TCPListener) Accept() (c Conn, err os.Error)

参数说明同DialTCP的参数一样。下面我们来实现一个简单的时间同步服务，监听7777端口	
...
上面的服务跑起来之后，他将会一直等待，直到有新的客户端请求到达，当有新的客户端请求到达并同意接受（Accept）该请求的时候，他会反馈当前的时间信息。值得注意的是for循环这边的代码，当有错误发生时，直接continue了，而不是退出，因为在跑服务器端代码的时候，当有错误发生的情况下最好是由服务端负责记录错误，同时当前发来请求的客户端会直接报错后退出，而不会影响到当前服务器的整个服务。

上面的代码执行的时候是单任务的，不能同时接收多个请求，那么该如何改造以使它支持多并发呢？Go里面有一个goroutine机制，请看下面改造后的代码
...
通过把业务处理独立到函数`handleClient`，我们就可以实现多并发执行了。这样看上去是不是很帅，增加`go`关键词就实现了服务端的多并发，从这个小例子也可以看出goroutine的强大之处。
...
第一个函数用来设置超时时间，客户端和服务器端都适用，当超过设置的时间后被控制的链接就失效。
